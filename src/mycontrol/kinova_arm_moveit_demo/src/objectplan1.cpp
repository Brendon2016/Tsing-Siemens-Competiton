...
int main(int argc, char **argv)
{
	...
	for(it = ObjectPositions.begin();it != ObjectPositions.end();it++)
	{
		...
		string name=(*it).name;
		cout<<name<<endl;
		if(name.compare("centrum_4")==0 || name.compare("centrum_5")==0 || name.compare("centrum_6")==0)
		{
		...
		target_pose4.position.x = target_pose8.position.x - 0.08 * i;
		i++;
